...
//        jScrollBar.setValue(jScrollBar.getMaximum());//设置垂直滚动条位置  
		Point p = new Point();
		p.setLocation(0, IntoFrame.getInst().getOrderPanel().getProductOrderListPanel().getPreferredSize().getHeight());
		IntoFrame.getInst().getOrderPanel().getScrollPane().getViewport().setViewPosition(p);
	}
	...
	public void updateTotal() {
		...
		IntoFrame.getInst().getOrderPanel().getTotalPanel().setTotal(text);
	}
