...
public class UserPanel extends JPanel{
	...
	public UserPanel(String title) {
		...
		table = new MyTable(UserProvider.getTitle(), UserProvider.getListValue(), 400, 300);
	...
	private void addButtonListener() {
		UserDialog dialog = new UserDialog(table, 1, title + "-" + addButton.getText());
	...
	private void alertButtonListener() {
		...
		UserDialog dialog = new UserDialog(table, 2, title + "-" + alertButton.getText());
		dialog.setVisible(true);
	}
	private void delButtonListener() {
		...
		int id = (Integer) table.getTable().getValueAt(selectRow, 0);
		User user = UserProvider.getInst().getUser(id);
		if(user != null) {
			if(UserProvider.getInst().del(user)) {
				table.getTableModel().removeRow(selectRow);
			}
		}
	}
